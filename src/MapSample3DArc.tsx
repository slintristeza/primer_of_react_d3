...
class MapSample extends Component<MapFile, MapState> {
  ...
  draw = async (ctx:CanvasRenderingContext2D) => {
    ...
    const pathGenerator = d3.geoPath<any, GeoPermissibleObjects>()
    ...
    const cityFeature = topojson
      .feature(citydata, citydata.objects[this.props.objectsname]);
    const cityFeatures:Feature[] = (cityFeature.type === "FeatureCollection") ?
      cityFeature.features : [cityFeature];
  ...
  private createCity(cityFeatures:Feature[],
    projection: d3.GeoProjection, ctx: CanvasRenderingContext2D) {
    const [min, max] = d3.extent(cityFeatures, (f):number => {
      return (f.properties) ? f.properties.POP_MAX : 0;
    });
    const scale = (min !== undefined && max !== undefined) ? d3.scaleLinear().domain([min, max]).range([0, 10]) : null;
    if (!scale) return;
  ...
  private createArcLine(cityFeatures:Feature[],
    projection: d3.GeoProjection, ctx: CanvasRenderingContext2D) {
    ...
    cityFeatures.forEach((f, i, d) => {
      if (f.geometry.type === "Point" && f.properties) {
        if (f.properties.POP_MAX < 5000000)
          return;
        ...
        if (target !== i && targetF
          ...
          const start = projection([
            f.geometry.coordinates[0],
            f.geometry.coordinates[1]]);
          const mid1 = sky(inter(.33));
          const mid2 = sky(inter(.76));
  ...
  private createLand(ctx: CanvasRenderingContext2D,
  ...
  private createSea(ctx: CanvasRenderingContext2D,
    pathGenerator: d3.GeoPath<any, d3.GeoPermissibleObjects>) {
